...
#      It defaults to s3://sagemaker-demo-samples/${image}/input/model.tar.gz
#      FIRST TIME USERS UPDATE THIS DEFAULT.
#
...
#      FIRST TIME USERS SHOULD UPDATE THIS DEFAULT.
#
# EXAMPLE USAGE:
...
image=$1
s3_model_location=${2:-s3://sagemaker-demo-samples/${image}/input/model.tar.gz} # FIRST TIME USERS UPDATE DEFAULTS
sagemaker_service_role=${3:-AmazonSageMaker-ExecutionRole-20171204T150334}      # FIRST TIME USERS UPDATE DEFAULTS
user=default
...
echo "Model uploaded to " ${s3_model_location}
...
$(aws ecr get-login --region ${region} --no-include-email --profile ${user})
...
endpoint_name=$image'-'$(date '+%Y%m%d-%H%M%S')
